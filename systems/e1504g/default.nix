...
}:
{
  imports = [
    ...
    # Charge limit module. This chassis exposes the standard asus-nb-wmi
  ...
  # below (on engaging it would restore the firmware's absurd RAPL
  ...
  #   - Wi-Fi (01:00.0) is left at power/control=on by iwlwifi itself,
  ...
  home-manager.users.kyandesutter =
    { pkgs, lib, ... }:
    {
      ...
      # 563 MB, beeper 385 MB, plus messages), which is most of what drives
