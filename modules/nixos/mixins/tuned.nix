...
}:
{
  config = lib.mkIf config.kyan.desktop.enable {
    ...
    # Boot into the declared default. tuned-ppd persists the selected profile
    # to /etc/tuned/ppd_base_profile and, at startup, that file beats
    ...
    # balanced" from the shell's battery popout would pin the host to that
    # tier on every subsequent boot, with nothing in the UI saying why (the
    # inverse bit the e1504g on 2026-09-01, when it still defaulted to
    # balanced and a performance pick outlived the boot). A profile picked
    # from the popout lasts until the next boot, no longer.
    systemd.services.tuned-ppd.serviceConfig.ExecStartPre = [
      "${pkgs.writeShellScript "tuned-ppd-reset-base-profile" ''
        printf '%s\n' ${lib.escapeShellArg config.services.tuned.ppdSettings.main.default} \
          > /etc/tuned/ppd_base_profile
      ''}"
    ];
  };
}
