{ lib, pkgs, ... }:
{
  ...
  networking.networkmanager = {
    ...
    # are iwlwifi, but the e1504g's card fails every PSK join under iwd
    # ("Operation failed" from iwctl, 2026-09-09), so that host overrides this
    # back to wpa_supplicant. NM's wifi.powersave knob does not reach iwd, so
    # the g815's iwlwifi power_save=0 module option is what keeps powersave
    # off there.
